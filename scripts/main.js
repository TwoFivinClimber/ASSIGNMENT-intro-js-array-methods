...
const search = (event) => {
  ...
  const searchList = referenceList.filter(
    (item) =>
      item.title.toLocaleLowerCase().includes(eventLC) ||
      item.author.toLocaleLowerCase().includes(eventLC)
  );
...
const buttonFilter = (event) => {
  ...
  if (event.target.id.includes('cartFilter')) {
    const inDatCart = referenceList.filter((item) => item.inCart);
    renderCards(inDatCart);
  }
